# Personal Portfolio Website

A modern and responsive personal portfolio website built using HTML, CSS, and JavaScript.  
This portfolio showcases my projects, skills, education, and contact information.

## 🚀 Features

- Responsive Design
- Dark/Light Theme Toggle
- Modern UI with Animations
- Project Showcase Section
- Contact Form
- Mobile Friendly Navigation
- Social Media Links

## 🛠️ Technologies Used

- HTML5
- CSS3
- JavaScript
- PHP
- MySQL

## 📂 Projects Included

### 1. HTML Portfolio
A minimalist personal website built with semantic HTML.

### 2. Algorithm Trading Bot with Python
A trading bot using Python with technical analysis and trading strategies.

### 3. Blood Bank Management System
A blood bank management project using PHP and MySQL.

## 🎓 Education

**B.Tech in Computer Science and Engineering**

Relevant Subjects:
- Web Development
- Data Structures & Algorithms
- Artificial Intelligence
- Software Engineering

## 📸 Preview

Add screenshots of your portfolio here.

## 🔗 Live Demo

Add your deployed website link here.

## 📬 Contact

- LinkedIn: https://www.linkedin.com/in/anjali-sindhwal-225166276/
- GitHub: https://github.com/Anjali020905

## ⚡ How to Run

1. Clone the repository

```bash
git clone https://github.com/Anjali020905/personal_profile1.git
